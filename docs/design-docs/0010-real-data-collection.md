...
- TDnet は開示が 0 件の日（休日）に一覧の表が無く「開示された情報はありません」と出る。空として扱う
- 証券コードには 4 桁目が英字のもの（`264A` など）がある。TDnet の 5 桁コードの照合は `^\d[0-9A-Z]{3}\d$`
- 実データでの初回: 27 銘柄 × 243 本の日足を約 12 秒で遡り、`collect all` は約 45 秒（ニュース 2,539 件）。`detect run` で 4 ルールすべてが評価され、6 件のシグナル（直近高値比 4、取得単価比 2）が出た
- 開示の種別バッジは重大度と混同しないよう `badge-primary`（紺の薄い面）を DESIGN.md に追加した
