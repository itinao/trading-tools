...
1. `detect run` の `as_of` の既定を「株価の最新日」に変える（0005 の supersede）でよいか
2. 日足の遡りは 1 年でよいか（`below_ma200` に 201 営業日 ≒ 10 か月が必要。余裕を見て 1 年）
3. ダッシュボードの銘柄詳細にニュース・開示を出すのを 0010 に含めてよいか（0011 まで待つ案もある）
