...
- 減衰と窓の「経過日数」は日付単位（当日 = 0）にした。時刻込みだと当日の判定でも少し減衰し、テストで値が安定しないため
- `detect run` の `as_of`（株価の最新日）より後に出たニュース・開示の判定は、その日のスコアに含めない（休日に出たニュースは翌営業日のスコアに入る）
- スキルの手順を実データで通した: 未判定 40 件（開示 2 + ニュース 38）を判定し、`record` → `detect run` → ダッシュボードで判定バッジ・スコア・上書きが動くことを確認。ソフトバンク（9434）・オリックス（8591）はプロ野球の記事が大半で `irrelevant` になる（0009 R2 のとおり）。銘柄ごとの検索語の上書きは今後の課題
- `schedule install` は利用者の LaunchAgents を書き換えるため、エージェントは勝手に実行しない。利用者が実行する
- ダッシュボードの判定の上書きは `sentiment` のみ（`impact` / `direction` は CLI の `assess override` で）
