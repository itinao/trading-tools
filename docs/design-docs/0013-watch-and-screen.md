...
1. 割安の既定（PER ≤ 12、PBR ≤ 1.0、配当利回り ≥ 3.0%、時価総額 ≥ 300 億円）でよいか
2. 攻めのルールは **ウォッチ銘柄のみ**（保有への買い増しは対象外）でよいか
3. スクリーニングの既定の母集団は **プライム + スタンダード**（グロース・ETF・REIT は除く）でよいか
4. ウォッチの `stance`（candidate / review / pass）でよいか
