...
| `margin_deterioration` | ファンダ | 営業利益率が前年比で悪化（閾値は M2 で決める） | 収益性の低下。持つ理由を再確認 | 年次の財務諸表（Yahoo、0009） | M2 |
| `equity_ratio_drop` | ファンダ | 自己資本比率の前年比の低下（閾値は M2 で決める） | 財務健全性の低下 | 年次の財務諸表（Yahoo、0009） | M2 |
...
| `growth_streak` | ファンダ（攻め） | 増収増益が N 年連続 | 質が良い。買い候補 | 年次の財務諸表（Yahoo、0009） | M4 |
...
- 決算データ（売上、営業利益、自己資本比率など）は本文 §11 のデータソース候補に無かった。S1 で検証した結果、無料で取れるのは年次（Yahoo、4 年分）のみ。四半期の時系列は J-Quants（有料）が必要で、契約しないと決めた（0009）
