...
- 損益・変化率の色を **楽天証券式（赤 = 上昇、緑 = 下落）** に変更する（現状は欧米式）。重大度の赤とは意味が異なるため、重大度はバッジ（面つき）で示して区別する
...
| 上昇・下落の色 | 楽天証券式（赤 = 上昇、緑 = 下落） | 欧米式（緑 = 上昇）: 利用者が毎日見る楽天証券のアプリと逆になり、読み違える。重大度の赤との混同はバッジの形で防ぐ |
...
| 論点 | 決定 |
| --- | --- |
| DESIGN.md の置き場 | `apps/dashboard/`（見た目を持つのはダッシュボードだけ） |
| デザインの方向 | 「静かな計器盤」。テーマの細部は実装側に一任 |
