...
| | |
| --- | --- |
| 状態 | 完了（2026-09-20）。M0〜M5 と S1 がすべて完了 |
...
| M2 | ニュース・開示とスコア | 「悪いニュースでスコアが下がる」が動く | 完了（2026-09-20） |
| M3 | AI助言 | 「何をすべきか」が出る | 完了（2026-09-20） |
| M4 | 攻め | 「見過ごし」を減らせる | 完了（2026-09-20） |
| M5 | 振り返り | 判断の改善に使える | 完了（2026-09-20） |
...
- [x] 保有銘柄のニュース・開示が日次で保存される（`collect all`、0010）
- [x] エージェントが `assess` で判定を付け、CLI 経由で保存される（スキル `.agents/skills/assess`、0011）
- [x] 悪い判定が付くとスコアが下がり、閾値を超えるとシグナルが出る（`news_negative` / `score_low` など、テストと実データで確認）
- [x] 判定に根拠が残り、ダッシュボードで人が上書きできる
- [x] 日次実行の環境が決まり（launchd、`pnpm schedule install`）、実行失敗はログとダッシュボードの「株価が古い」バナーで分かる。登録は利用者が行う
...
- 0012: `advise`（入力の範囲、出力の形、重複防止、助言の品質を後から評価する手がかり）

### 完了の定義

- [x] シグナルが出た翌朝、対応する助言つきアクションがダッシュボードにある（`morning` スキルで assess → detect → advise）
- [x] 同じシグナルに助言が二重に付かない（`(signal_id, origin)` の一意制約と `already_advised`）
- [x] 助言に、参照した事実（株価、ニュース、開示）へのリンクがある（`references`、1 件以上必須）
...
- 0013: ウォッチと攻めのルール、`screen`（母集団は JPX の上場銘柄一覧）

### 完了の定義

- [x] ウォッチ銘柄を追加すると、株価（1 年分を自動遡り）・指標・財務・ニュースが集まりスコアが付く
- [x] ウォッチ銘柄が割安・成長・売られすぎの条件を満たすと「買い検討」のアクションが出る（`valuation_cheap` / `growth_streak` / `oversold_quality`）
- [x] 条件を指定してスクリーニングし（`pnpm screen run`）、ダッシュボードの結果からウォッチに追加できる
...
- 0014: 振り返り（タイムライン、履歴、review ツール、retrospect スキル）

### 完了の定義

- [x] 銘柄詳細で、過去のシグナルとアクションと当時のスコアを時系列で見られる（タイムライン、株価とスコアの折れ線）
- [x] 「見送り」にしたアクションを後から一覧できる（`/history`、判断時の株価とその後の変化つき）
