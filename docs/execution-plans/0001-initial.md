...
- マイルストーンに着手する前に、その変更の Design Doc を書いて承認を得る
...
| S1 | データソース検証（並行） | 株価・ニュース・開示の取得元が決まる | 未着手 |
...
- 0003: `import-holdings`（CSV の列定義、文字コード、銘柄・保有のテーブル）
- 0004: `collect`（株価の取得と保存。Provider 抽象、モック実装、S1 の結果で差し替え）
- 0005: `detect`（下落ルール、シグナル・アクションのテーブル、閾値の設定方法）
- 0006: `dashboard`（画面遷移、アクション一覧と銘柄詳細の最小構成）

4本は多いので、0003+0004（入力側）、0005+0006（出力側）の2本にまとめてもよい。
...
候補ごとに小さなスクリプトで実際に取得し、結果を Design Doc（0004 の補足、または独立した 0007）に記録する。

| 種類 | 候補 | 確認すること |
| --- | --- | --- |
| 株価・指標 | Yahoo Finance（非公式） | 日本株の取得可否、PER 等の指標、利用規約 |
| 株価・指標・業績予想 | J-Quants | 無料プランの遅延、有料プランの価格、業績予想の形 |
| 適時開示 | TDnet | 銘柄別に取れるか、ページ構造、更新タイミング |
| ニュース | Google News RSS | 銘柄名検索の精度、ノイズの量 |

### 完了の定義

- [ ] 株価・指標の取得元が決まり、`collect` のモックを実装に差し替えられる
- [ ] ニュース・開示の取得元が決まる（取れない場合は「開示のみ」に縮小することを決める）
- [ ] 各取得元の利用条件（規約・レート制限・費用）が Design Doc に記録されている
...
- 0007: `collect`（ニュース・開示）
- 0008: `assess` スキルと判定のデータ構造（根拠の引用、人による上書き）
- 0009: スコア（算出式、重み、閾値、設定の外出し）
...
- 0010: `advise`（入力の範囲、出力の形、重複防止、助言の品質を後から評価する手がかり）
...
- 0011: ウォッチと攻めの閾値
- 0012: `screen`（対象銘柄の母集団をどう用意するか — S1 の結果に依存）
...
1. M1 の Design Doc を4本にするか2本にまとめるか
2. S1 を M0 の直後に集中してやるか、M1 と並行にするか（並行なら M1 はモックで進む）
3. M2 と M4 の順序。困りごとの順（守り → 攻め）で M2 を先にしているが、M4 は M1 だけに依存するので先にもできる
4. 各マイルストーンの大きさ感。エージェントに渡す単位として大きすぎないか
