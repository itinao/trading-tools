...
| M1 | 守りの最小経路 | 「検知 → 確認」が一本通る | 実装中（0003 承認） |
...
- 0003: 入力側 — `import-holdings`（CSV の列定義、文字コード、銘柄・保有のテーブル）と `collect`（株価の取得と保存。Provider 抽象、モック実装、S1 の結果で差し替え）
- 0004: 出力側 — `detect`（下落ルール、シグナル・アクションのテーブル、閾値の設定方法）と `dashboard`（画面遷移、アクション一覧と銘柄詳細の最小構成）
...
- 0006: `collect`（ニュース・開示）
- 0007: `assess` スキルと判定のデータ構造（根拠の引用、人による上書き）
- 0008: スコア（算出式、重み、閾値、設定の外出し）
