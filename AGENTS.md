...
**実行計画 0001（M0〜M5）完了。実行計画 [0002](docs/execution-plans/0002-operations.md)（運用フェーズの改善）の O1（ダッシュボードの情報設計、0015）と O7（フォントとアイコン、0016）、O8（配信、0017）完了。ロゴ・文言・スマホ幅は 0015 の補足。** 次の項目に着手する前に Design Doc を書いて承認を得る。
...
pnpm dashboard:build && pnpm dashboard:start   # 本番ビルド（apps/dashboard/dist）を apps/dashboard/server.mjs（srvx）で配信。PORT / HOST で変更可
...
1. Design Doc を書いて承認を得る（既存の Design Doc の範囲内なら、その「補足」に追記でよい）
2. **モックを作って見せる**: `apps/dashboard/mock/` に静的 HTML を置く。`tokens.css` と `styles.css` を読み込み、フォント・アイコンは Google Fonts の CDN でよい（モックだけ）。ヘッドレス Chrome で撮る:
   ```bash
