...
- **スマホ（640px 未満）**: ナビはタブバー（アイコンの下に小さくラベル、4 項目が横に収まる）。左右の余白 16px。**横スクロールは使わない**。表は主要な列だけ出し（`col-wide` を隠す。保有なら 銘柄 / 株価 / 前日比 / 損益）、残りは銘柄の詳細で見る。銘柄名の列（`col-name`）に余った幅を渡す。アクションのカードは見出しとボタンを縦に積む
...
白地、右に 1px の罫線。上にロゴ、中に 4 項目（アイコン + ラベル、間隔 `sm`。アクティブでも同じ間隔。1 行の説明 `label-sm` は名詞句）、下に **サイト全体の更新情報**（`sidebar-status`: 株価の鮮度と最終収集、`label-sm`、アイコン付き、古い・失敗は `warn` の文字色）。アクティブは左に 3px の `primary` の縦線と `primary-container` の面。未対応の件数は `badge-critical`。画面固有の更新情報はサイドバーではなくヘッダ帯の右に置く。
...
### 実行の要約 / 条件（`run-meta` / `criteria`）
スクリーナーの実行（プリセットの名前と意味、実行日時、母集団 → 件数）を `on-surface-variant` の 1 行に並べる。条件は `criteria` のチップ（`surface-container` の面、角丸 `sm`、等幅数字）で 1 条件 1 チップ。JSON は出さない。
...
- Do: 画面の文は **です・ます調** で書く（「〜します」「〜ありません」「〜してください」）。1 文ごとに改行し、1 文を長くしない。ボタン・タブ・表のヘッダ・バッジは名詞か短い動詞（「対応した」「外す」）でよい
- Do: ページのタイトルの下に説明を置く。1 文目は「何が出ているか」、2 文目は「何をするか」
- Do: 空状態には次にやるコマンドを書く（例: 「`pnpm import-holdings run` で取り込みます」）
- Do: 設定の識別子（プリセット名 `value` など）や JSON をそのまま出さない。日本語の名前と言葉にした条件を出し、識別子は補助として添える
