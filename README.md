...
**M2（ニュース・開示とスコア）完了。** 保有 CSV の取り込み → 実データの収集 → AI によるニュース・開示の判定 → スコアと 10 種のルールで検知 → ダッシュボードで確認、が動いている。日次実行は launchd に登録できる。次は M3（AI 助言）。
...
朝はエージェント（Claude Code / codex）に「assess を実行して」と言うと、ニュース・開示の判定が付き、スコアと検知に反映される。日次実行の自動化は `pnpm schedule install`。
